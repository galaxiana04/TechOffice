...
<head>
    ...
    <title>{{ $progressreport[0]->unit }}</title>
        ...
        <table id="example4" class="table table-bordered table-hover">
            <thead>
                <tr>
                    ...
                    <th scope="col">Status Sinkron</th>
                </tr>
            </thead>
            <tbody>
                @php
                ...
                @foreach ($progressreport as $index => $progressReport)
                    <tr>
                        ...
                        <td id="startreleasedate_{{ $progressReport->id }}_{{ $index }}">
                            {{ $progressReport->startreleasedate ? $progressReport->startreleasedate : '' }}
                        </td>
                        <td id="deadlinerelease_{{ $progressReport->id }}_{{ $index }}">
                            {{ $progressReport->deadlinereleasedate ? $progressReport->deadlinereleasedate : '' }}
                        </td>
                        <td id="realisasi_{{ $progressReport->id }}_{{ $index }}">
                            {{ $progressReport->realisasidate ? $progressReport->realisasidate : '' }}
                        </td>
                        ...
                        <td id="in_schedule_{{ $progressReport->id }}_{{ $index }}">
                            ...
                            @if ($progressReport->status == 'RELEASED')
                                @if ($startReleaseDate && $deadlineReleaseDate && $realisasiDate)
                                    @if ($realisasiDate->lt($startReleaseDate) || $realisasiDate->between($startReleaseDate, $deadlineReleaseDate))
                                        Tidak Telat
                                    @else
                                        Telat
                                    @endif
                                ...
                                @endif
                            @else
                                @if ($startReleaseDate && $deadlineReleaseDate)
                                    @if ($today->lt($startReleaseDate) || $today->between($startReleaseDate, $deadlineReleaseDate))
                                        Tidak Telat
                                    @else
                                        Telat
                                    @endif
